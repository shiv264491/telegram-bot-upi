...
async def buy_app(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_photo(
        ...
        caption=(
            ...
            f"3️⃣ Payment ke baad UTR number copy karein\n"
...
async def handle_utr(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(utr) < 8 or not any(c.isdigit() for c in utr):
        await update.message.reply_text(
            "❌ *Galat UTR!* Dobara sahi UTR paste karein:",
            parse_mode="Markdown"
        )
        return
    ...
    if ADMIN_ID != 0:
        ...
        try:
            ...
            # Admin notify fail hua toh auto approve
            await _send_app_link(context.bot, user.id, utr)
    else:
        # No admin — auto approve
        await _send_app_link(context.bot, user.id, utr)


async def _send_app_link(bot, user_id: int, utr: str):
    try:
...
async def admin_approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = int(parts[1])
    utr = parts[2]
    await _send_app_link(context.bot, user_id, utr)
    await query.edit_message_text(
        f"✅ *Approved!* User `{user_id}` ko link bhej diya!\nUTR: `{utr}`",
        parse_mode="Markdown"
    )
